...
export const getStockItems = async (axios: AxiosInstance, stockId: string): Promise<StockWithStockItems[]> =>
  apiCall(async () => await axios.get(stockApi.getItems(stockId)), 'Skladové položky se nepovedlo najít.')
...
export const getStockAllowances = async (
  ...
  apiCall(async () => await axios.get(stockApi.getAllowances(params)), 'Spotřebu se nepovedlo najít.')
