...
export const patchUpdateVisit = async (
  ...
  apiCall(async () => await axios.patch(visitApi.getById(visitId), visitData), 'Úprava návštěvy neproběhla úspěšně.')

export const deleteVisit = async (axios: AxiosInstance, visitId: string): Promise<string> =>
  apiCall(async () => await axios.delete(visitApi.delete(visitId)), 'Nepovedlo se smazat návštěvu.')

export const patchUpdateVisitStatus = async (axios: AxiosInstance, data: { visitId?: string; status: boolean }) =>
  apiCall(async () => await axios.patch(visitApi.updateStatus(), data), 'Úprava stavu návštěvy neúspěšná.')
