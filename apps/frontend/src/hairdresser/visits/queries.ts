...
export const useVisitStatusMutation = () => {
  ...
  return useMutation<
    { visitId?: string; status: boolean },
    Error,
    { visitId?: string; status: boolean } | undefined
  >({
...
export const useUpdateVisitMutation = (visitId: string | undefined) => {
  ...
  return useMutation<string, Error, VisitDetailFormType>({
    ...
    onSuccess: () => {
      ...
      const { from, to }: PersistentFiltersType['visits']['dashBoardVisitOverView']['dates'] =
        persistentFilters ? JSON.parse(persistentFilters).visits.dashBoardVisitOverView.dates : '';
